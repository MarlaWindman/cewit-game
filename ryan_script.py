...
def audio_do():
    ...
    with sr.Microphone() as source:
        ...
        for _ in range(100) :
            try:
                audio=recognizer.listen(source,timeout=.25,phrase_time_limit=3)
...
def speech_text_handler(text:str):
    text=text.lower()

    if 'north' in text:
        command_handler('north')
            ...
            or 'call' in text \
            or 'sauce' in text:
        command_handler('south')

    elif 'east' in text:
        command_handler('east')

    elif 'wes' in text \
            or 'west' in text:
        command_handler('west')
